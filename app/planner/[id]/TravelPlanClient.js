// app/planner/[id]/TravelPlanClient.js
'use client';
...
const ErrorState = ({ error, onRetry }) => (
  <div className="text-center py-12">
    ...
    <div className="mt-6 flex flex-col sm:flex-row justify-center gap-4">
      ...
      <a href="/planner" className="btn btn-primary py-2 px-4 bg-blue-600 text-white rounded-md hover:bg-blue-700 flex items-center justify-center">
        <FiArrowLeft className="mr-2" /> 여행 계획 페이지로 돌아가기
      </a>
    </div>
  </div>
);
...
export default function TravelPlanClient({ planId }) {
  ...
  // Fetch travel plan data with retry capability
  ...
  // Loading state is now handled by the Suspense boundary
      ...
      <section className={isMobile ? "mb-4" : "mb-8"}>
        <div className={isMobile ? "" : "bg-white dark:bg-gray-900 rounded-lg shadow-lg"}>
          <TravelItinerary 
            ...
            travelPlanId={planId} 
            isOwner={isOwner}
            onUpdatePlan={handleTravelPlanUpdate}
          />
        </div>
      </section>
    </div>
